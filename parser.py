...
def p_statement_return(p):
    ...
    # Infer or ask for return type based on what's being returned
    ret_val = p[2]
    
    # Store a special key for return type
    if "__return_type__" not in symbol_table:
        inferred = infer_type(ret_val)
        if inferred:
            symbol_table["__return_type__"] = inferred
        else:
            # Ask user for return type
